...
over labeled answer strings, not by generating and string-matching. This is the
...
harness uses (lm-eval, OpenCompass, etc.).
    ...
    MC2: normalize log-probs over all mc2_targets, sum probability of the
...
length-normalize (per Lin et al.; lm-eval does the same).
"""
...
class MCResult:
    ...
    mc2: float           # total normalized probability assigned to correct answers
...
def mc1_mc2(
    ...
    Args:
        ...
        mc1_correct_idx: index in mc1_choices that's labeled correct
    ...
    # MC2 = sum P(correct) / sum P(all); normalize logprobs via softmax
